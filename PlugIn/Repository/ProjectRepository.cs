...
namespace PlugIn.Repository
{
								...
								JwtTokenProvider jwtToken, PlugInDbContext context) : IProjectRepository
    {
        ...
        public async Task<IdentityResult> Register(RegisterForm form)
        {
            ...
            //await ConfirmEmailAsync(user.Email, token);
		...
		public async Task<BaseResponse> SendEmailAsync(string email)
		{
			var user = await _userManager.FindByEmailAsync(email);
			if (user == null)
				return new BaseResponse { StatusCode = 400, Message = "user not found." };

			string verifyemail = email; // "paruyr@fmail.com";
			var verifytoken = "654410"; // await GenerateEmailConfirmationToken(user);
